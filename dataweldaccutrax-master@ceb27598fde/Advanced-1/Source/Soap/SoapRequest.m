...
-(void)handleError:(NSError*)error{
	...
	if([self.handler respondsToSelector: onerror]) {
		((void(*)(id, SEL, id))objc_msgSend)(self.handler, onerror, error);
        //[[NSObject alloc] init];
	} else {
		if(self.defaultHandler != nil && [self.defaultHandler respondsToSelector:onerror]) {
			((void(*)(id, SEL, id))objc_msgSend)(self.defaultHandler, onerror, error);
            //[[NSObject alloc] init];
		}
	}
	if(self.logging) {
	...
	if([fault hasFault]) {
		if(self.action == nil) {
			...
			if(self.handler != nil && [self.handler respondsToSelector: self.action]) {
				((void(*)(id, SEL, id))objc_msgSend)(self.handler, self.action, fault);
			} else {
...
//        NSLog(@"self.action: %@",self.action);
        ...
        NSLog(@"self.handler: %@",self.handler);
//        NSLog(@"self.action: %@",output);
        NSLog(@"output: %@",output);
		if(self.handler != nil && [self.handler respondsToSelector: self.action]) {
			 ((void(*)(id, SEL, id))objc_msgSend)(self.handler, self.action, output);
